...
def login():
    form = LoginForm()
    if form.validate_on_submit():
        check_user_login = User.query.filter_by(email=form.email.data).first()
        check_pass_hash = check_password_hash(check_user_login.password, form.password.data)
        if check_pass_hash and check_user_login:
            login_user(check_user_login)
            return redirect(url_for("get_all_posts"))
        else:
